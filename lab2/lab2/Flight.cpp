...
void SetDurationInMinutes(Flight& flight, const int durationInMinutes)
{
	...
	{
		string message = "Число " + to_string(durationInMinutes) +
			" не является положительным\nи не может "
			"определять продолжительность полета самолета";
		char buf[255];
		strcpy_s(buf, message.c_str());
		throw exception(buf);
	}
...
void SetDistanceInMeters(Flight& flight, const int distanceInMeters)
{
	...
	{
		string message = "Число " + to_string(distanceInMeters) +
			" не является положительным\nи не может "
			"определять дистанцию полета самолета";
		char buf[255];
		strcpy_s(buf, message.c_str());
		throw exception(buf);
	}
